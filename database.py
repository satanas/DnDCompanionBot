...
class Database:
    ...
    def set_turn_index(self, campaign_id, turn_index):
        return self.firebase_db.patch('/campaigns', campaign_id, data={'turn_index': turn_index}, params={'auth': FIREBASE_API_SECRET})

    def set_turns(self, campaign_id, turns):
        return self.firebase_db.patch('/campaigns', campaign_id, data={'turns': turns, 'turn_index': '0'}, params={'auth': FIREBASE_API_SECRET})

    def get_character_info(self, chat_id, username):
        results = self.firebase_db.get('/', 'characters', params={'orderBy': '\"chat_id\", \"username\"',
    ...
    def set_dm(self, chat_id, user_id, username):
        return self.firebase_db.patch('/campaigns', params={'orderBy': '\"chat_id\"',
                                                            'equalTo': chat_id,
                                                            'auth': FIREBASE_API_SECRET},
